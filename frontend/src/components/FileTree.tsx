...
export default function FileTree({ files }: Props) {
  if (files.length === 0) {
    return <div className="text-text-muted text-sm p-4">No files yet.</div>;
  }
    ...
    <div className="p-2 text-sm font-mono">
      {files.map((f) => {
        ...
        return (
          <div
            key={f}
            className="flex items-center gap-1.5 py-0.5 text-text-secondary hover:text-text-primary hover:bg-bg-hover rounded px-1"
